...
def true_reward_function(action_idx, context, true_thetas, delta_skill_focus, delta_overall, noise_std=0.1):
    ...
    reward = 0.6 * delta_skill_focus[action_idx] + 0.4 * delta_overall + linear_part + np.random.normal(0, noise_std)
    return reward
...
if __name__ == "__main__":
    ...
    prev_skill_scores = np.zeros(len(action_space))
    ...
    for t, simulation in enumerate(data):
        ...
        context = np.hstack(([baseline_feats[feat] for feat in baseline_feats if feat != 'rubric_score']))

        action_idx, action_name = ts_adapter.select_action(context)

        print(f"Step {t}: Selected action '{action_name}' for context {context}")

        delta_skill_focus = baseline_feats["rubric_vector"] - prev_skill_scores
